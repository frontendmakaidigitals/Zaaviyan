      ...
      <motion.article className="container font-[700] font-Primary text-center text-2xl flex flex-wrap justify-center">
        {para.split(" ").map((elem, idx) => (
          <span key={idx} className="overflow-hidden inline-block">
            ...
            >
                {idx === 10 && <StarFour weight="fill" className="mr-2 text-lime-400" />}
                {idx === 25 && <CirclesThree weight="fill" className="mr-2 text-lime-400" />}
                {idx === 33 && <LineSegments weight="fill" className="mr-2 text-lime-400" />}
              {elem}
            </motion.span>
          </span>
        ))}
